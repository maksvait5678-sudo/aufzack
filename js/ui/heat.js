...
export function renderHeat(heatEl, topic, states, peek) {
  ...
  topic.matrix.rows.forEach(cs => {
    ...
    topic.matrix.cols.forEach(g => {
      ...
      const v = related.reduce((acc, c) => acc + (st(c.id) ? Math.min(st(c.id).b, 4) / 4 : 0), 0) / related.length;
      const a = topic.matrix.value(cs.k, g.k);
      const label = peek ? `<span>${a}</span>` : `<span class="pct">${Math.round(v * 100)}%</span>`;
      h += `<div class="hc" title="${cs.label} ${g.label}"><div class="fill" style="background:${peek ? topic.colors[a] : 'var(--prog)'};transform:scaleX(${peek ? 1 : v})"></div>${label}</div>`;
    });
  });
  heatEl.innerHTML = h;
}
